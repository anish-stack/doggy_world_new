...
const LabTestAll = () => {
    ...
    // SWR hook to fetch labtest products
    const { data, error, mutate } = useSWR(
        `http://localhost:8000/api/v1/LabTest-products?page=${currentPage}`,
        fetcher
    );
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`http://localhost:8000/api/v1/LabTest-delete-product/${id}`, {
                method: 'DELETE',
            });
    ...
    const navigateToEdit = (id) => {
        navigate(`/dashboard/edit-labtest-product/${id}`);
    };

    const navigateToCreate = () => {
        navigate('/dashboard/create-labtest-product');
    };
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold">Lab Tests Products Management</h1>
            ...
            {isLoading && <div className="text-center py-8">Loading Lab Test products...</div>}

            {error && (
                <div className="text-center py-8 text-red-500">
                    Error loading labtest products. Please try again.
                </div>
            )}
            ...
            {data && data.data && data.data.length === 0 && (
                <div className="text-center py-8">
                    No Lab tests  found. Create a new one to get started.
                </div>
            )}

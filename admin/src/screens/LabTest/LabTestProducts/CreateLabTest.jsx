...
const CreateLabTest = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/LabTest-product`, {
                method: 'POST',
                body: formDataToSend,
            });
            ...
            if (response.ok) {
                setAlert({
                    show: true,
                    type: 'success',
                    message: 'Lab Test product added successfully!'
                });

                toast.success('Lab Test product added successfully!')
                // Redirect back to list after short delay
                setTimeout(() => {
                    router('/dashboard/all-labtest');
                }, 1000);
            } else {
                toast.error(result.message || 'Failed to add Lab Test product')

                throw new Error(result.message || 'Failed to add Lab Test product');
            }
        } catch (error) {
            toast.error(error.message || 'Failed to add Lab Test product')

            setAlert({
                show: true,
                type: 'error',
                message: error.message || 'Error adding Lab Test product'
            });
        } finally {
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-1 gap-2 mt-2">
                                ...
                                {vaccineTypesData?.data?.length === 0 && (
                                    <p className="text-gray-500">No Lab  types available</p>
                                )}
                            </div>
                        </div>
                    </CardContent>

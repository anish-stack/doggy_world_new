...
const AllCollectionLabTest = () => {
  ...
  const { data, error, mutate } = useSWR(
    `http://localhost:8000/api/v1/list-all-LabTest-types?page=${page}&limit=${limit}${search ? `&search=${search}` : ''}`,
    fetcher
  )
  ...
  const handleDeleteVaccine = async () => {
    try {
      const response = await axios.delete(`http://localhost:8000/api/v1/remove-LabTest-type/${selectedVaccine._id}`)
    ...
    <div className="container mx-auto py-6">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle>Lab Test Collection</CardTitle>
            <CardDescription>Manage your Lab Test types here.</CardDescription>
          </div>
          <Button onClick={handleOpenCreateModal}>
            <Plus className="mr-2 h-4 w-4" /> Add Lab Test type
          </Button>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between mb-6">
            <div className="relative w-full max-w-sm">
              ...
              <Input
                placeholder="Search Lab test Types..."
                className="pl-8"
                value={searchInput}
...
export default AllCollectionLabTest
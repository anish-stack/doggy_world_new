...
const AllBakeryCategories = () => {
  ...
  const handleDeleteDesign = async () => {
    try {
      ...
      const response = await axios.delete(`${API_URL}/delete-pet-bakery/${selectedDesign._id}`);
    ...
    <div className="container mx-auto py-6">
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <CardTitle className="text-2xl font-bold">Pet Bakery Categories</CardTitle>
          <Button
        ...
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            <div className="relative w-72">
              ...
              <Input
                placeholder="Search Pet Category..."
                value={search}
                    ...
                    {paginatedData.length === 0 ? (
                      ...
                      paginatedData.map((design) => (
                        <TableRow key={design._id}>
                          <TableCell className={'capitalize'}>{design?.position}</TableCell>
                          <TableCell className="text-center">
                            {design.imageUrl && (
                              <img
                                src={design.imageUrl.url}
                                alt={design.name}
                                className="h-12 w-12 object-contain rounded-md mx-auto"
                              />
                            )}
                          </TableCell>
                          <TableCell className={'capitalize'}>{design?.title || 'N/A'}</TableCell>
                          <TableCell >
                            <span className={`px-2 py-1 capitalize rounded-full text-xs font-medium  bg-violet-100 text-gray-800`}>
                              {design?.tag || "N/A"}
                            </span>
                          </TableCell>
                          <TableCell className="text-center">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${design.active
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete the flavor "{selectedDesign?.title}".
...
export default AllBakeryCategories
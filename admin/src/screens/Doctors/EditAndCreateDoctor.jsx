...
const EditAndCreateDoctorModel = ({ isOpen, onClose, doctor, onSuccess }) => {
    ...
    useEffect(() => {
        if (doctor) {
            setFormData({
                ...
                is_best: doctor.is_best !== undefined ? doctor.is_best : true,
            })
            if (doctor.imageUrl?.url) setImagePreview(doctor.imageUrl.url)
        } else {
            setFormData({
                name: '',
                position: '',
    ...
    const handleToggleActive = (checked) => {
        setFormData({ ...formData, is_best: checked })
    }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            payload.append('is_best', formData.is_best)
            ...
            let response
            if (doctor) {
                response = await axios.put(`${API_URL}/doctors/edit/${doctor._id}`, payload, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                })
            } else {
                response = await axios.post(`${API_URL}/doctors/create`, payload, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                })
            }

            if (response.status === 200 || response.status === 201) {
                toast.success(`Doctor ${doctor ? 'updated' : 'created'} successfully`)
                onSuccess()
                onClose()
            }
        } catch (err) {
            ...
            <DialogContent className="sm:max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-4">
                    ...
                    <div className="flex items-center justify-between">
                        <Label htmlFor="is_best">Best Doctor</Label>
                        <Switch id="is_best" checked={formData.is_best} onCheckedChange={handleToggleActive} />
                    </div>

                    <DialogFooter className="pt-4">
                        <Button type="button" variant="outline" onClick={onClose} disabled={isSubmitting}>Cancel</Button>
                        <Button type="submit" disabled={isSubmitting} className="bg-blue-600 hover:bg-blue-700">
                            {isSubmitting ? 'Saving...' : doctor ? 'Update' : 'Create'}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
...
export default EditAndCreateDoctorModel
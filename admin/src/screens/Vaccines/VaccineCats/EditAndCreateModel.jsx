    ...
    try {
      ...
      if (isEditMode) {
        ...
        response = await axios.put(
          `${API_URL}/update-vaccine-type/${vaccine._id}`, 
          formDataToSend, 
          {
            headers: {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{isEditMode ? 'Edit Vaccine' : 'Create New Vaccine'}</DialogTitle>
          <DialogDescription>
            {isEditMode 
              ? 'Update the details of this vaccine type.' 
              : 'Fill in the details to create a new vaccine type.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting}>
              ...
              {isEditMode ? 'Update Vaccine' : 'Create Vaccine'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>

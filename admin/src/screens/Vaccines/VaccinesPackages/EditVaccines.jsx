...
const EditVaccines = () => {
    ...
    const { data, error, mutate } = useSWR(
        id ? `http://localhost:8000/api/v1/vaccine-product/${id}` : null,
        fetcher
    );
    ...
    useEffect(() => {
        if (data && data.data) {
            ...
            setFormData({
                ...
                VaccinedInclueds: Array.isArray(vaccineData?.VaccinedInclueds) && vaccineData.VaccinedInclueds.length > 0
                    ? vaccineData.VaccinedInclueds.join(',')
                    : '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.put(`${API_URL}/vaccine-update-product/${id}`, formDataToSend, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
    
            toast.success(data.message || 'Vaccine product updated successfully!');
    
            setAlert({
                show: true,
                type: 'success',
                message: data.message || 'Vaccine product updated successfully!'
            });
    ...
    return (
        <div className="container mx-auto p-4">
            <div className="flex items-center mb-6">
                ...
                <h1 className="text-2xl font-bold">Edit Vaccine Product</h1>
            </div>

...
const SingleVaccineBookings = () => {
  ...
  const fetchBooking = async () => {
    setLoading(true)
    try {
      const response = await axios.get(`http://localhost:8000/api/v1/single-vaccine-orders?id=${id}`)
      ...
      if (response.data.data.bookingRating) {
        setRatingValue(Number.parseInt(response.data.data.bookingRating) || 5)
      }
      ...
      setError("Failed to fetch Vaccination booking details")
      toast.error("Failed to fetch Vaccination booking details")
      setLoading(false)
      console.error(err)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8000/api/v1/delete-vaccine-orders/${booking._id}`)

      toast.success("Booking deleted successfully")
      setDeleteOpen(false)
      navigate("/Vaccination") // Go back to list
    } catch (err) {
  ...
  const handleCancel = async () => {
    ...
    try {
      await axios.put(`http://localhost:8000/api/v1/cancel-vaccine-orders?id=${booking._id}&status=Cancelled`)
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8000/api/v1/update-vaccine-status`, {
        id: booking._id,
        status: newStatus,
      })
  ...
  const goBack = () => {
    navigate("/dashboard/Vaccination-booking")
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2">
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-2xl font-bold">Vaccinations Booking Details</CardTitle>
            ...
            <CardContent>
              <Tabs defaultValue="details" className="mt-4">
                <TabsList className="grid w-full grid-cols-3">
                  ...
                  <TabsTrigger value="therapy">Vaccination Details</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="details" className="mt-4">
                        ...
                        {booking.rescheduledDate ? (
                          <div className="mt-2">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              Originally: {format(new Date(booking.selectedDate), "MMMM dd, yyyy")} at {booking.time}
                            </p>
                          </div>
                        ) : (
                          <p className="text-muted-foreground mt-1">
                            {booking.selectedDate ? format(new Date(booking.selectedDate), "MMMM dd, yyyy") : "Not scheduled"} at{" "}
                          ...
                          {booking.payment && (
                            <div className="mt-3 space-y-1">
                              <p className="text-sm">
                                ...
                                {formatCurrency(Number.parseInt(booking.payment.amount) / 100)}
                              </p>
                              <p className="text-sm">
                              ...
                              <p className="text-sm">
                                ...
                                {format(new Date(booking.payment.createdAt), "MMM dd, yyyy 'at' hh:mm a")}
                              </p>
                            </div>
                          )}
                        </div>
                  ...
                  {/* Review Section */}
                  {booking.bookingRating && booking.bookingRating !== "0" && (
                    <div className="mt-6 p-4 bg-muted rounded-lg">
                      ...
                      <div className="flex items-center mt-2">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`h-5 w-5 ${i < Number.parseInt(booking.bookingRating) ? "text-yellow-500 fill-yellow-500" : "text-gray-300"
                              }`}
                          />
                        ))}
                        <span className="ml-2 text-sm font-medium">{booking.bookingRating}/5</span>
                      </div>
                ...
                <TabsContent value="therapy" className="mt-4">
                  {booking.vaccine ? (
                    <div className="space-y-6">
                      {/* Vaccine Images */}
                      {booking.vaccine.image && booking.vaccine.image.length > 0 && (
                        <Card>
                          <CardContent className="pt-6">
                            ...
                            <Carousel className="w-full">
                              <CarouselContent>
                                {booking.vaccine.image.map((image, index) => (
                                      ...
                                      <Card>
                                        <CardContent className="flex aspect-square items-center justify-center p-2">
                                          <img
                                            ...
                                            alt={`${booking.vaccine.title} - Image ${index + 1}`}
                      ...
                      <Card>
                        <CardContent className="pt-6">
                          <div className="space-y-4">
                            <div>
                              <h3 className="text-xl font-bold">{booking.vaccine.title}</h3>
                              <div className="flex flex-wrap gap-2 mt-2">
                                {/* Base price */}
                                {booking?.bookingType === 'Home' ? (
                                  <Badge variant="outline" className="text-sm">
                                    {formatCurrency(booking.vaccine?.home_price_of_package)}
                                  </Badge>
                                ) : (
                                  <Badge variant="outline" className="text-sm">
                                    {formatCurrency(booking.vaccine?.price)}
                                  </Badge>
                                )}


                                {/* Conditional: Home or Clinic discounted price */}
                                {booking?.bookingType === 'Home' ? (
                                  booking.vaccine?.home_price_of_package_discount && (
                                    <Badge variant="outline" className="bg-green-50 text-green-700 text-sm">
                                      Discounted: {formatCurrency(booking.vaccine.home_price_of_package_discount)}
                                    </Badge>
                                  )
                                ) : (
                                  booking.vaccine?.discount_price && (
                                    <Badge variant="outline" className="bg-green-50 text-green-700 text-sm">
                                      Discounted: {formatCurrency(booking.vaccine.discount_price)}
                                    </Badge>
                                  )
                                )}
                                ...
                                {/* Popular badge */}
                                {booking.vaccine?.popular && (
                                  <Badge className="bg-amber-100 text-amber-800 text-sm">Popular</Badge>
                                )}
                              </div>
                              <div>
                                <h4 className="font-medium">Vaccine Title</h4>
                                <p className="text-muted-foreground mt-1">{booking.vaccine.title}</p>
                              </div>
                            </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button className="w-full flex items-center gap-2" variant="outline" onClick={() => navigate(`/dashboard/vaccination-booking/${booking?._id}/make-schedules`)}>
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>How would you rate your Vaccination experience?</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={statusOpen} onOpenChange={setStatusOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>Change the current status of this Vaccination booking.</DialogDescription>
          </DialogHeader>
...
export default SingleVaccineBookings

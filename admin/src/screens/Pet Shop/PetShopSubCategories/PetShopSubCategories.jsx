...
const PetShopSubCategories = () => {
  ...
  const { data, error, mutate, isValidating } = useSWR(
    `${API_URL}/petshop-sub-category?page=${page}&limit=${limit}${search ? `&search=${search}` : ''}`,
    fetcher
  )
  ...
  const handleDeleteCategory = async () => {
    try {
      ...
      const response = await axios.delete(`${API_URL}/petshop-sub-category/${selectedCategory._id}`)
    ...
    <div className="container mx-auto py-6 px-4">
      <Card className="shadow-md">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <CardTitle className="text-2xl font-bold">Pet Shop Sub Categories</CardTitle>
            <CardDescription>Manage your pet shop sub categories</CardDescription>
          </div>
                      ...
                      <TableCell>
                        {category.parentCategory && category.parentCategory.length > 0 ? (
                          <div className="flex flex-wrap gap-2">
                            {category.parentCategory.map((item, index) => (
                              <span
                                key={index}
...
export default PetShopSubCategories

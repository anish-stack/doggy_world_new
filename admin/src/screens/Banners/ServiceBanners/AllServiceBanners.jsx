...
export default function AllServiceBanners() {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/service-banner`);
  ...
  const deleteBanner = async () => {
    ...
    try {
      const response = await axios.delete(`${API_URL}/service-banner/${bannerToDelete._id}`);
      if (response.data.success) {
  ...
  const handleEditClick = (banner) => {
    navigate(`/dashboard/service-banners/home/edit/${banner._id}`, { state: { banner } });
  };

  const handleCreateNew = () => {
    navigate("/dashboard/service-banners/home/create");
  };
    ...
    <div className="container mx-auto p-6">
      <Card className="w-full">
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-2xl font-bold">All Services Banners</CardTitle>
            <CardDescription>
              Manage banners displayed on the services and product pages
            </CardDescription>
          </div>

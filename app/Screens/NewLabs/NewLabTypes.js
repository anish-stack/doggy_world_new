...
const VaccineTypeCard = ({ navigation, item }) => {
    ...
    return (
        <TouchableOpacity onPress={() => navigation.navigate('labtests', { type: item.title, id: item._id })} activeOpacity={0.7} style={styles.card}>
...
const NewLabTypes = () => {
    ...
    const fetchVaccineTypes = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${API_END_POINT_URL_LOCAL}/api/v1/list-all-LabTest-types`);
    ...
    return (
        <>
            <TopHeadPart title={'Pick Your Lab test Spot!'} fnc={handleCallPress} />

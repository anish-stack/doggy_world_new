const TypeOfLabCollection = require("../../models/LabsTest/TypeOfLabCollection");
const { deleteFile } = require("../../middleware/multer");
const { uploadSingleFile, deleteFileCloud } = require("../../utils/upload");


exports.createTypeOfLabTest = async (req, res) => {
    ...
    try {
        ...
        const positionTaken = await TypeOfLabCollection.findOne({ position });
        if (positionTaken) {
            if (file) await deleteFile(file.path);
            return res.status(400).json({
                success: false,
        ...
        const newType = await TypeOfLabCollection.create({
            title,
            description,
            is_active,
            position,
        ...
        if (file) await deleteFile(file.path);
        await redis.del('type-of-labtest:all');

        return res.status(201).json({
            success: true,
            message: "Type of LabTest created successfully",
            data: newType
        });
    } catch (error) {
        console.error(error);
...
exports.getAllTypeOfLabTests = async (req, res) => {
    try {
        const redis = req.app.get('redis');
        const { search = '', page = 1, limit = 10 } = req.query;
        console.log(req.query)

        const cached = await redis.get('type-of-labtest:all');
        if (cached) {
        ...
        const total = await TypeOfLabCollection.countDocuments(query);
        const types = await TypeOfLabCollection.find(query)
            .sort({ position: 1 })
            .skip((page - 1) * limit)
            .limit(parseInt(limit));

        await redis.set('type-of-labtest:all', JSON.stringify(types));
...
// Get Single
exports.getSingleTypeOfLabTest = async (req, res) => {
    try {
        const { id } = req.params;
        const type = await TypeOfLabCollection.findById(id);
...
// Update
exports.updateTypeOfLabTest = async (req, res) => {
    try {
        const redis = req.app.get('redis');
        const { id } = req.params;
        const file = req.file;
        ...
        if (Number(position) && Number(position) !== type.position) {
            ...
            const positionTaken = await TypeOfLabCollection.findOne({ position:pos });
            if (positionTaken) {
                if (file) await deleteFile(file.path);
                return res.status(400).json({
                    success: false,
...
// Delete
exports.deleteTypeOfLabTest = async (req, res) => {
    try {
        const redis = req.app.get('redis');
        const { id } = req.params;
        ...
        await TypeOfLabCollection.findByIdAndDelete(id);
        await redis.del('type-of-labtest:all');

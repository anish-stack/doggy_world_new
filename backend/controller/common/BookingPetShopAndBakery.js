...
exports.getMyAllBookingDetailsShopBooking = async (req, res) => {
  try {
    ...
    if(!petId){
      return res.status(401).json({
        success:false,
        message:"please login to access these details"
      })
    }

    const findOrder = await BakeryAndShopBooking.find({ petId })
      .populate('petId', 'petname petOwnertNumber petbreed')
      .populate('deliveryInfo', 'street city state zipCode country')
      .populate('paymentDetails', 'razorpay_order_id amount payment_status createdAt razorpay_payment_id')
      .populate('items.itemId').sort({ createdAt: 1 })
...
exports.getMySingleBookingDetailsShopBooking = async (req, res) => {
  try {
    ...
    if(!id){
      return res.status(401).json({
        success:false,
        message:"please id to access these orders details"
      })
    }
...
exports.deletePetShopAndBakeryOrder = async (req, res) => {
  try {
    ...
    const deletedOrder = await BakeryAndShopBooking.findByIdAndDelete(id);
